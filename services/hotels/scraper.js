...
export async function scrapeHotels(socket, dateFrom, dateTo, hotelUrl, defaultUrls) {
    ...
    const siteName = 'Hotels.com';
    const siteKey = 'hotels';
    ...
    try {
        ...
        page.on('response', async (response) => {
            ...
            // Specifically looking for Hotels.com review APIs and Expedia GraphQL endpoints fallback
            if (reqUrl.includes('/api/v3/reviews') || reqUrl.includes('reviews-summary') || reqUrl.includes('graphql') || reqUrl.includes('propertyReviews')) {
                ...
                try {
                    ...
                    // Directly pull standard properties if it's the specific v3 API
        ...
        let targetUrl = url;
        if (!targetUrl.includes('pwaDialog')) {
            const baseUrl = targetUrl.split('?')[0];
            targetUrl = `${baseUrl}?pwaDialog=reviews-property-reviews-wrapper-1`;
        }
        ...
        // ── 3: Wait for content and aggressively trigger modal ──
        emit('Waiting for review content and triggering dialog...');
        
        // Wait for the page or dialog to settle
        await page.waitForTimeout(2000);

        // Hotels.com blocks clicks with overlays. Use javascript to click the actual "See all X reviews" button
        emit('Attempting to force-click "See all reviews" button to trigger API...');
        await page.evaluate(() => {
            ...
            if (reviewBtns.length > 0) {
                // Click the last one or the first one
                reviewBtns[0].click();
            }
        });
        ...
        try {
            ...
            emit('Status: DOM check passed to API interceptor loop.');
        }
